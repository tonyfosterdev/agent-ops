 ...
 *
 * The coding agent specializes in code analysis, debugging, and repairs.
 * It receives context from the log-analyzer agent via the logFindings parameter
...
/**
 * Build the findings context section if log findings are available.
 */
function buildFindingsContext(logFindings: unknown): string {
  if (!logFindings) return '';
...
/**
 * Generate the coding agent system prompt.
 *
 * @param context - Optional context with log findings from log-analyzer agent
 * @returns Complete system prompt string
 */
export function codingSystemPrompt(context?: CodingPromptContext): string {
  const findingsContext = buildFindingsContext(context?.logFindings);
